...
export const REFLECTION_DECK = Object.freeze([
  makeCard({
    ...
    visualBriefZh: "一把旧木椅朝向半开的门，门帘被风吹进空房；构图仅含一个主要主体与一个异常关系，保留 65% 安静留白；采用蓝调夜幕手绘动画语言，暖色只用于关系节点",
    visualBriefEn: "An old wooden chair faces a half-open door as the curtain blows into an empty room; show one main subject and one unusual relationship with 65% quiet negative space; use a hand-drawn animated language in blue night tones, with warmth only at the relationship point",
    ...
    prohibitedClaimsZh: ["对方一定会回来", "等待会带来好运"],
    prohibitedClaimsEn: ["The other person will definitely return", "Waiting will bring good luck"],
  }),
  makeCard({
    ...
    visualBriefZh: "透明玻璃瓶中悬着一小片下雨的灰云，瓶外桌面干燥；构图仅含一个主要主体与一个异常关系，保留 65% 安静留白；采用蓝调夜幕手绘动画语言，暖色只用于关系节点",
    visualBriefEn: "A small grey cloud rains inside a clear glass bottle while the table outside stays dry; show one main subject and one unusual relationship with 65% quiet negative space; use a hand-drawn animated language in blue night tones, with warmth only at the relationship point",
    ...
    prohibitedClaimsZh: ["情绪预示坏事", "压抑一定来自童年"],
    prohibitedClaimsEn: ["This emotion predicts something bad", "Suppression always comes from childhood"],
  }),
  makeCard({
    id: "state-full-cup",
    category: "state",
    ...
    visualBriefZh: "一只几乎溢出的陶杯仍接住上方最后一滴水；构图仅含一个主要主体与一个异常关系，保留 65% 安静留白；采用蓝调夜幕手绘动画语言，暖色只用于关系节点",
    visualBriefEn: "A ceramic cup close to overflowing still receives one last drop from above; show one main subject and one unusual relationship with 65% quiet negative space; use a hand-drawn animated language in blue night tones, with warmth only at the relationship point",
    ...
    prohibitedClaimsZh: ["你必然会崩溃", "继续坚持就一定成功"],
    prohibitedClaimsEn: ["You will inevitably break down", "Continuing to endure guarantees success"],
  }),
  makeCard({
    id: "state-fog-window",
    category: "state",
    ...
    visualBriefZh: "一扇起雾的窗只被擦开很小一角，窗外轮廓模糊；构图仅含一个主要主体与一个异常关系，保留 65% 安静留白；采用蓝调夜幕手绘动画语言，暖色只用于关系节点",
    visualBriefEn: "Only a tiny corner of a fogged window has been wiped clear while the view beyond remains blurred; show one main subject and one unusual relationship with 65% quiet negative space; use a hand-drawn animated language in blue night tones, with warmth only at the relationship point",
    ...
    prohibitedClaimsZh: ["直觉一定正确", "答案会自动出现"],
    prohibitedClaimsEn: ["Intuition is always correct", "The answer will appear on its own"],
  }),
  makeCard({
    ...
    visualBriefZh: "一个站立人物朝左，地面影子却朝右；构图仅含一个主要主体与一个异常关系，保留 65% 安静留白；采用蓝调夜幕手绘动画语言，暖色只用于关系节点",
    visualBriefEn: "A standing figure faces left while the shadow on the ground points right; show one main subject and one unusual relationship with 65% quiet negative space; use a hand-drawn animated language in blue night tones, with warmth only at the relationship point",
    ...
    prohibitedClaimsZh: ["对方在欺骗你", "影子代表真实人格"],
    prohibitedClaimsEn: ["The other person is deceiving you", "The shadow reveals a true personality"],
  }),
  makeCard({
    ...
    visualBriefZh: "一座窄桥从近岸伸向远岸，却在抵达前缺少最后一段；构图仅含一个主要主体与一个异常关系，保留 65% 安静留白；采用蓝调夜幕手绘动画语言，暖色只用于关系节点",
    visualBriefEn: "A narrow bridge reaches from the near bank toward the far bank but lacks its final span; show one main subject and one unusual relationship with 65% quiet negative space; use a hand-drawn animated language in blue night tones, with warmth only at the relationship point",
    ...
    prohibitedClaimsZh: ["对方不爱你", "停止联系就会得到答案"],
    prohibitedClaimsEn: ["The other person does not love you", "Ending contact will give you the answer"],
  }),
  makeCard({
    ...
    visualBriefZh: "一个人撑着明显过大的旧伞，伞柄系着通向画外的细线；构图仅含一个主要主体与一个异常关系，保留 65% 安静留白；采用蓝调夜幕手绘动画语言，暖色只用于关系节点",
    visualBriefEn: "A person holds an oversized old umbrella whose handle is tied to a thread leading out of frame; show one main subject and one unusual relationship with 65% quiet negative space; use a hand-drawn animated language in blue night tones, with warmth only at the relationship point",
    ...
    prohibitedClaimsZh: ["帮助者一定在控制你", "依赖本身是错误"],
    prohibitedClaimsEn: ["The helper is definitely controlling you", "Depending on others is inherently wrong"],
  }),
  makeCard({
    ...
    visualBriefZh: "一把钥匙已插入锁孔却齿形不合，锁与钥匙之间留有微小偏差；构图仅含一个主要主体与一个异常关系，保留 65% 安静留白；采用蓝调夜幕手绘动画语言，暖色只用于关系节点",
    visualBriefEn: "A key is inserted into a lock but its teeth do not fit, leaving a slight mismatch between them; show one main subject and one unusual relationship with 65% quiet negative space; use a hand-drawn animated language in blue night tones, with warmth only at the relationship point",
    ...
    prohibitedClaimsZh: ["双方天生不合", "沟通一定能解决一切"],
    prohibitedClaimsEn: ["The two people are inherently incompatible", "Communication will solve everything"],
  }),
  makeCard({
    ...
    visualBriefZh: "一盏未点燃的手提灯旁放着一根可用的火柴；构图仅含一个主要主体与一个异常关系，保留 65% 安静留白；采用蓝调夜幕手绘动画语言，暖色只用于关系节点",
    visualBriefEn: "An unlit lantern sits beside a match that is ready to use; show one main subject and one unusual relationship with 65% quiet negative space; use a hand-drawn animated language in blue night tones, with warmth only at the relationship point",
    ...
    prohibitedClaimsZh: ["机会马上到来", "你拥有尚未觉醒的神秘力量"],
    prohibitedClaimsEn: ["An opportunity is about to arrive", "You possess a hidden supernatural power"],
  }),
  makeCard({
    ...
    visualBriefZh: "一根完整绳索上只有一个正在松开的结，绳结未断；构图仅含一个主要主体与一个异常关系，保留 65% 安静留白；采用蓝调夜幕手绘动画语言，暖色只用于关系节点",
    visualBriefEn: "A single knot loosens along an intact rope without breaking it; show one main subject and one unusual relationship with 65% quiet negative space; use a hand-drawn animated language in blue night tones, with warmth only at the relationship point",
    ...
    prohibitedClaimsZh: ["必须彻底切断", "放手后一切都会变好"],
    prohibitedClaimsEn: ["You must cut the connection completely", "Letting go will make everything better"],
  }),
  makeCard({
    ...
    visualBriefZh: "高墙上的正门紧闭，画面边缘却有一扇小侧门透出微光；构图仅含一个主要主体与一个异常关系，保留 65% 安静留白；采用蓝调夜幕手绘动画语言，暖色只用于关系节点",
    visualBriefEn: "The main gate in a high wall is closed while a small side door at the edge gives off a faint light; show one main subject and one unusual relationship with 65% quiet negative space; use a hand-drawn animated language in blue night tones, with warmth only at the relationship point",
    ...
    prohibitedClaimsZh: ["替代路径必然成功", "有人会暗中帮助你"],
    prohibitedClaimsEn: ["An alternative route is guaranteed to succeed", "Someone will secretly help you"],
  }),
  makeCard({
    ...
    visualBriefZh: "潮水退去后露出三层石阶，原本覆盖的下一阶刚刚显现；构图仅含一个主要主体与一个异常关系，保留 65% 安静留白；采用蓝调夜幕手绘动画语言，暖色只用于关系节点",
    visualBriefEn: "Three stone steps emerge as the tide recedes, with the next covered step only just becoming visible; show one main subject and one unusual relationship with 65% quiet negative space; use a hand-drawn animated language in blue night tones, with warmth only at the relationship point",
    ...
    prohibitedClaimsZh: ["等待会自动解决问题", "潮汐预示未来变化"],
    prohibitedClaimsEn: ["Waiting will solve the problem automatically", "The tide predicts future change"],
  }),
]);
